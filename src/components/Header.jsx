...
const Header = () => {
    ...
    const login = async () => {
        if (!user) {
            ...
            localStorage.setItem("user", JSON.stringfy(providerData[0]));
        } else {
            setIsMenue(!isMenu)
        }
    };
        ...
        <header className='fixed z-50 w-screen  p-3 px-4 md:p-6 md:px-16 bg-primary'>
            ...
            <div className="hidden md:flex w-full h-full items-center justify-between">
                <Link to={'/'} className='flex items-center gap-2'>
                    ...
                    <p className="text-headingColor text-xl font-bold">Foodies Zone</p>
                </Link>
                            ...
                            onClick={login} />
                        {
                            isMenu && (
                                <motion.div
                                    ...
                                    {user && user.email === "rushikeshpise13@gmail.com" && (
                                        <Link to={'/createItem'}>
            ...
            <div className=" flex items-center justify-between md:hidden w-full h-full">
                ...
                <Link to={'/'} className='flex items-center gap-2'>
                    <img src={Logo} className="w-8 object-cover" alt="logo" />
                    <p className="text-headingColor text-x1 font-bold">Foodies Zone</p>
                </Link>
                            ...
                            <motion.div
                                ...
                                <ul className="flex flex-col">
                                    ...
                                    transition-all ease-in-out cursor-pointer hover:bg-slate-100 px-4 py-2"
                                            onClick={() => setIsMenue(false)}> Home</li>
                                    </Link>
                                    <li className="text-base text-textColor hover:text-headingColor duration-100
                                transition-all ease-in-out cursor-pointer hover:bg-slate-100 px-4 py-2"
                                        onClick={() => setIsMenue(false)}> Menu</li>
                                </ul>

                                <p className="m-2 p-2 rounded-md shadow-md flex items-center bg-gray-200 gap-3 cursor-pointer hover:bg-gray-300 
